...
#define REF_CUT_BREAK_CLOSE_RETIO 0.5	//ƒuƒŒ[ƒL‚ð•Â‚¶‚é”»’èŒW”@‚Pƒmƒbƒ`‘¬“x‚Æ‚Ì”ä—¦
...
void CCrane::Ac() {
	...
	if(!motion_break[ID_HOIST]) a_ref[ID_HOIST] = 0.0;
		...
		if (v_ref[ID_HOIST] > 0.0) a_ref[ID_HOIST] = pspec->accdec[ID_HOIST][FWD][ACC];//³“]‰Á‘¬
		else a_ref[ID_HOIST] = pspec->accdec[ID_HOIST][REV][DEC];//‹t“]Œ¸‘¬
	}
	else if ((v_ref[ID_HOIST] - v0[ID_HOIST]) < -accdec_cut_spd_range[ID_HOIST]) {
		if (v_ref[ID_HOIST] > 0.0) a_ref[ID_HOIST] = pspec->accdec[ID_HOIST][FWD][DEC];//³“]Œ¸‘¬
		else a_ref[ID_HOIST] = pspec->accdec[ID_HOIST][REV][ACC];//‹t“]‰Á‘¬
	}
	else{
		a_ref[ID_HOIST] = 0.0;
	}
	...
	if (!motion_break[ID_GANTRY]) a_ref[ID_GANTRY] = 0.0;
		...
		if (v_ref[ID_GANTRY] > 0.0) a_ref[ID_GANTRY] = pspec->accdec[ID_GANTRY][FWD][ACC];//³“]‰Á‘¬
		else a_ref[ID_GANTRY] = pspec->accdec[ID_GANTRY][REV][DEC];//‹t“]Œ¸‘¬
	}
	else if ((v_ref[ID_GANTRY] - v0[ID_GANTRY]) < -accdec_cut_spd_range[ID_GANTRY]) {
		if (v_ref[ID_GANTRY] > 0.0) a_ref[ID_GANTRY] = pspec->accdec[ID_GANTRY][FWD][DEC];//³“]Œ¸‘¬
		else a_ref[ID_GANTRY] = pspec->accdec[ID_GANTRY][REV][ACC];//‹t“]‰Á‘¬
	}
	else {
		a_ref[ID_GANTRY] = 0.0;
	}
	...
	if (!motion_break[ID_BOOM_H]) a_ref[ID_BOOM_H] = 0.0;
		...
		if (v_ref[ID_BOOM_H] > 0.0) a_ref[ID_BOOM_H] = pspec->accdec[ID_BOOM_H][FWD][ACC];//³“]‰Á‘¬
		else a_ref[ID_BOOM_H] = pspec->accdec[ID_BOOM_H][REV][DEC];//‹t“]Œ¸‘¬
	}
	else if ((v_ref[ID_BOOM_H] - v0[ID_BOOM_H]) < -accdec_cut_spd_range[ID_BOOM_H]) {
		if (v_ref[ID_BOOM_H] > 0.0) a_ref[ID_BOOM_H] = pspec->accdec[ID_BOOM_H][FWD][DEC];//³“]Œ¸‘¬
		else a_ref[ID_BOOM_H] = pspec->accdec[ID_BOOM_H][REV][ACC];//‹t“]‰Á‘¬
	}
	else {
		a_ref[ID_BOOM_H] = 0.0;
	}
	...
	if (!motion_break[ID_SLEW]) a_ref[ID_SLEW] = 0.0;
		...
		if (v_ref[ID_SLEW] > 0.0) a_ref[ID_SLEW] = pspec->accdec[ID_SLEW][FWD][ACC];//³“]‰Á‘¬
		else a_ref[ID_SLEW] = pspec->accdec[ID_SLEW][REV][DEC];//‹t“]Œ¸‘¬
	}
	else if ((v_ref[ID_SLEW] - v0[ID_SLEW]) < -accdec_cut_spd_range[ID_SLEW]) {
		if (v_ref[ID_SLEW] > 0.0) a_ref[ID_SLEW] = pspec->accdec[ID_SLEW][FWD][DEC];//³“]Œ¸‘¬
		else a_ref[ID_SLEW] = pspec->accdec[ID_SLEW][REV][ACC];//‹t“]‰Á‘¬
	}
	else {
		a_ref[ID_SLEW] = 0.0;
	}
	
	//‰Á‘¬“xŒvŽZ@“––ÊŽw—ß‚É‘Î‚µ‚ÄˆêŽŸ’x‚êƒtƒBƒ‹ƒ^‚ð“ü‚ê‚éŒ`‚ÅŒvŽZi«—ˆ“I‚Éƒgƒ‹ƒNŽw—ß‚©‚ç‚Ì“±oŒŸ“¢j
...
// Šeƒ‚[ƒVƒ‡ƒ“‚ÌƒuƒŒ[ƒLó‘Ô‚ðƒZƒbƒg
